...
def test_client_updates_existing_sku_file_by_id_with_put():
    opener = RecordingOpener(
        [FakeResponse({"Id": 520, "SkuId": 251, "ArchiveId": 155467, "IsMain": False, "Label": "Image 03"})]
    )
    client = _client(opener)

    updated = client.update_sku_file(
        251,
        520,
        {
            "IsMain": False,
            "Label": "Image 03",
            "Name": "82YU00XYLM_03.jpg",
            "Url": "https://mcp.artos.pe/vtex-images/signed-new",
        },
    )

    assert updated["Id"] == 520
    request, timeout = opener.calls[0]
    assert request.full_url.endswith("/api/catalog/pvt/stockkeepingunit/251/file/520")
    assert request.get_method() == "PUT"
    assert timeout == 30
    payload = json.loads(request.data.decode("utf-8"))
    assert payload["Name"] == "82YU00XYLM_03.jpg"
    assert payload["Url"].endswith("signed-new")
