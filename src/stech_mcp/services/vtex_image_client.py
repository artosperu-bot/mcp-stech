...
class VtexImageClient:
    ...
    def update_sku_file(
        ...
        parsed = int(sku_id)
        file_id = int(sku_file_id)
        if parsed <= 0 or file_id <= 0:
            raise ValueError("sku_id and sku_file_id must be positive")
        result = self._request(
            operation="update_sku_file",
            method="PUT",
            path=f"/api/catalog/pvt/stockkeepingunit/{parsed}/file/{file_id}",
            payload=payload,
        )
        if result is None:
            return {}
        if not isinstance(result, dict):
            raise VtexImageApiError(
                ...
                status=200,
                body=f"respuesta inesperada: {result!r}",
                url=f"{self.base_url}/api/catalog/pvt/stockkeepingunit/{parsed}/file/{file_id}",
            )
        return result
