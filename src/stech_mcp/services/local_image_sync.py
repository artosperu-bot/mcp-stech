...
class LocalImageSyncService:
    ...
    def sync(self, partnumber: str) -> dict[str, Any]:
        ...
        for position, path in discovered:
            ...
            # Position is part of the local image identity. The same binary may
            # intentionally be assigned to more than one VTEX position.
    ...
    def validate(self, partnumber: str) -> dict[str, Any]:
        # Filesystem is the source of truth for automatic VTEX updates. Re-scan
        # on every validation so newly added positions and changed binaries are
        # visible immediately instead of relying on stale DB metadata.
